...
int main(int argc, char* argv[]){
    ...
    FILE* test = fopen("L_test.txt","a+");
    struct reb_vec3d L1 = reb_simulation_angular_momentum(r);
    double L1_mag = sqrt(L1.x * L1.x + L1.y * L1.y + L1.z*L1.z);

    fprintf(test, "n before: %u\t", r->N);
    fprintf(test, "L vector components (x, y, z) = %e\t%e\t%e\t", L1.x, L1.y, L1.z);
    fprintf(test, "L magnitude = %e\t", L1_mag);
    fprintf(test, "\n");

    reb_simulation_integrate(r, 1e4);
    
    struct reb_vec3d L2 = reb_simulation_angular_momentum(r);
    double L2_mag = sqrt(L2.x * L2.x + L2.y * L2.y + L2.z*L2.z);

    fprintf(test, "n after: %u\t", r->N);
    fprintf(test, "L vector components (x, y, z) = %e\t%e\t%e\t", L2.x, L2.y, L2.z);
    fprintf(test, "L magnitude = %e\t", L2_mag);
    fprintf(test, "\n");
