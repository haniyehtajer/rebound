...
int main(int argc, char* argv[]){
    ...
    int n_emb = 154; //number of planetary embryos
    double m_emb[n_emb]; //mass of each embryo
    int ef = 1;
    double a_emb[n_emb]; // Same for embryos

    FILE *file_emb = fopen("cont_semis.txt", "r");
    for (int i = 0; i < n_emb; i++) {
        fscanf(file_emb, "%lf", &a_emb[i]);}
    // Close the file after reading
    fclose(file_emb);

    FILE *file_emb_mass = fopen("cont_masses.txt", "r");
    for (int i = 0; i < n_emb; i++) {
        fscanf(file_emb_mass, "%lf", &m_emb[i]);}
    // Close the file after reading
    fclose(file_emb_mass);
