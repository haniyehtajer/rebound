...
char TITLE[100] = "ueda_v5_";

double min_frag_mass = 0.5 * 4.405e-8;
...
//0=elastic bounce, 1=merger, 2=partial accretion, 3=partial erosion, 4=supercat
    ...
    fprintf(of, "%e\t", (r->particles[params->target].r));
    fprintf(of, "%e\t", (r->particles[params->target].x));
    fprintf(of, "%e\t", (r->particles[params->target].y));
    fprintf(of, "%e\t", (r->particles[params->target].z));
    fprintf(of, "%e\t", (r->particles[params->projectile].hash));
    fprintf(of, "%e\t", (r->particles[params->projectile].m));
    fprintf(of, "%e\t", (r->particles[params->projectile].r));
    fprintf(of, "%e\t", (r->particles[params->projectile].x));
    fprintf(of, "%e\t", (r->particles[params->projectile].y));
    fprintf(of, "%e\t", (r->particles[params->projectile].z));  
...
int main(int argc, char* argv[]){
    ...
    //This set of simulations is for a disk with same mass as Chambers 2013
    //But all bodies have equal mass and size
    //with m_emb = 1.5 * m_pl = 1.5 * 2.8e-8
    double rho = 5.05e6; //3 g/cm^3
    int n_emb = 150; //number of planetary embryos
    double m_emb = 4.405e-8; //mass of each embryo
    int ef = 1;
    double a_emb[n_emb]; // Same for embryos

    FILE *file_emb = fopen("ueda_2021_v5.txt", "r");
    ...
    for (int i=0; i<n_emb; i++){
        ...
        fprintf(of_dbcti, "%e\t", 0.33); //core frac of body
        fprintf(of_dbcti, "\n"); 
    }
